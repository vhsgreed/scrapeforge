...
Checked daily instead of by hand: Hacker News, books.toscrape.com,
quotes.toscrape.com and quotes.toscrape.com/js/ run in the
[live check](.github/workflows/live.yml) every morning (UTC), both from the
example configs and from configs generated by `scrapeforge init`. Those runs
use GitHub's datacenter IPs, which shops block far more often than home IPs,
so the live check covers sandbox sites only.
